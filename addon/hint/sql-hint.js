...
(function (mod) {
  ...
  var Pos = CodeMirror.Pos, cmpPos = CodeMirror.cmpPos , oldPos
  ...
  CodeMirror.registerHelper("hint", "sql", function (editor, options) {
    ...
    if (token.string.match(/^[.`\w@\u4e00-\u9fa5][\w\u4e00-\u9fa5]*$/)) {
      search = token.string;
      start = token.start;
      end = token.end;
    } else {
    ...
    if (search.charAt(0) == "." || search.charAt(0) == "`") {
      ...
      if (test(frontKey, 'FROM') || test(frontKey, 'JOIN') || test(frontKey, 'TABLE')) {
    ...
    function test(key,test){
      var re = new RegExp('^\\s?' + test + '\\s?$', 'gi');

      return re.test(key);
    }
    // console.log('search=[' + search + ']');
    // search为空
    var _search = search.replace(/(^\s*)|(\s*$)/g, '');
